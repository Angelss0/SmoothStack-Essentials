...
import com.ss.utopia.controllers.UserRoleController;
import com.ss.utopia.models.User;
import com.ss.utopia.util.ConnectionsManager;

public class AdminEmployeesView extends BaseAdminView {

    public AdminEmployeesView(Scanner scanner, View previousView) {
    ...
    public IOption add() {
        try {
            ...
            System.out.println(new UserRoleController().readAll().toString());

            return addModel("Employee", new String[] {
                "Given name",
                "Family name",
                "Username",
                "Email",
                ...
                return new UserController().createNewUser(freeId, args, User.EMPLOYEE);
    ...
    public IOption delete() {
        return deleteModel("Employee", new UserController(), (user) -> user.getRole().getId() == User.EMPLOYEE);
    }

    @Override
    public IOption read() {
        return readModel("employee", new UserController(), (user) -> user.getRole().getId() == User.EMPLOYEE);
    }
